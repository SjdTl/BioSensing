...
def mlp_train_test_measured(features, two_label=False, hidden_layer_1_nodes = 50, hidden_layer_2_nodes=30, print_messages = True, save_figures=True):
...
def mlp_eval_measured(wesad, measured, two_label=False, hidden_layer_1_nodes = 50, hidden_layer_2_nodes=30, print_messages = True, save_figures=True):
    ...
    # convert from integers to floats
    measured = measured.astype('float32')
    wesad = wesad.astype('float32')

    ####### Multi layer perceptron #######
    #Drop random feature
    measured_data = measured.drop(columns=['random_feature'])
    wesad_data = wesad.drop(columns=['random_feature'])

    #Redo numbering of subjects
    measured_data.loc[measured_data['subject'] == 16, 'subject'] = 1
    measured_data.loc[measured_data['subject'] == 17, 'subject'] = 12
    wesad_data.loc[wesad_data['subject'] == 16, 'subject'] = 1
    wesad_data.loc[wesad_data['subject'] == 17, 'subject'] = 12

    #Remove the subject and label collums and normalize
    measured_data_turncated = measured_data.drop(columns=['label', 'subject'])
    wesad_data_turncated = wesad_data.drop(columns=['label', 'subject'])
    scaler = StandardScaler()
    measured_data_scaled = pd.DataFrame(scaler.fit_transform(measured_data_turncated))
    measured_data_scaled = measured_data_scaled.join(measured_data['label'])
    measured_data_scaled = measured_data_scaled.join(measured_data['subject'])
    wesad_data_scaled = pd.DataFrame(scaler.fit_transform(wesad_data_turncated))
    wesad_data_scaled = wesad_data_scaled.join(wesad_data['label'])
    wesad_data_scaled = wesad_data_scaled.join(wesad_data['subject'])

    #Split into X and Y
    X_train = wesad.drop(columns=['label', 'subject']).to_numpy()
    Y_train = wesad['label'].to_numpy()
    x_test = measured.drop(columns=['label', 'subject']).to_numpy()
    y_test = measured['label'].to_numpy()
